...
import { 
  ...
  ShieldCheck, ScanLine, Upload, Receipt, Loader2, SmartphoneNfc, Info
} from 'lucide-react';
...
function AtomaConfirmContent() {
  ...
  const handleConfirmPayment = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(error.message || "خطای ارتباط با سرور رخ داد.");
    } finally {
          ...
          <div className="rounded-2xl border border-neutral-800/60 bg-[#0d0d0d]/50 p-6 sm:p-8">
            ...
            <div className="flex items-center gap-3.5 mb-8 pb-6 border-b border-neutral-800/60">
              <div className="flex h-12 w-12 items-center justify-center rounded-xl border border-neutral-800 bg-neutral-900 shadow-sm">
                <SmartphoneNfc className="h-6 w-6 text-[#FAD961]" />
              </div>
              <div>
                <h1 className="text-xl font-bold text-neutral-100 tracking-tight">AtomaPay Gateway</h1>
            ...
            <div className="flex flex-col sm:flex-row items-center gap-6 mb-8 bg-[#0a0a0a] rounded-xl border border-neutral-800/60 p-6">
              {/* QR Code Placeholder */}
              <div className="relative aspect-square w-32 bg-white rounded-xl overflow-hidden border border-neutral-200 p-2 shrink-0 flex items-center justify-center">
                <span className="text-gray-400 text-[10px] font-bold text-center">QR CODE<br/>HERE</span>
              </div>
              ...
              <div className="text-center sm:text-left space-y-3 w-full">
                <div>
                  ...
                  <p className="text-xl font-mono font-medium text-neutral-200 tracking-wider">0700 123 456</p>
                </div>
            ...
            <div className="rounded-xl bg-neutral-900/50 border border-neutral-800 p-5 space-y-2.5">
              ...
              <p className="text-xs text-neutral-400 leading-relaxed font-light">
                Please transfer exactly <strong className="text-neutral-200 font-medium">{finalPriceAfn} AFN</strong> via the AtomaPay app. Once completed, enter the Transaction ID (TID) and upload a screenshot of your receipt in the form.
              </p>
            </div>
        ...
        <div className="space-y-6 h-full">
          <form onSubmit={handleConfirmPayment} className="rounded-2xl border border-neutral-800/60 bg-[#0d0d0d]/50 p-6 sm:p-8 flex flex-col h-full">
            ...
            <div className="mt-8 pt-6 border-t border-neutral-800/60 flex flex-col sm:flex-row gap-3">
              {/* 🟢 دکمه کنسل نیز به صفحه چک‌اوت برمی‌گردد */}
              <button 
                type="button" 
                onClick={() => router.push(`/dashboard/checkout?plan=${packageName.toLowerCase()}`)} 
