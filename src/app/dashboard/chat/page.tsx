...
import { 
  ...
  User, Bot, Loader2, ArrowLeft, Terminal, MessageSquare, Plus, Menu, X, PanelLeftClose, PanelLeft, Download, Paperclip, MoreVertical, Trash2, Edit2, Pin, CheckCircle2, Copy, Check, MousePointer2
} from 'lucide-react';
...
const CodeBlock = ({ code }: { code: string }) => {
  ...
  return (
    <div className="bg-[#0A051A]/80 rounded-xl border border-white/10 overflow-hidden font-mono text-xs mt-3 shadow-2xl relative group/code backdrop-blur-md">
      <div className="flex items-center justify-between px-3 md:px-4 py-2.5 bg-black/40 border-b border-white/5 text-neutral-400">
        <span className="flex items-center gap-1.5 text-[10px] uppercase tracking-widest text-fuchsia-400 font-black">
          <Terminal className="w-3.5 h-3.5"/> Safi Code Engine
        </span>
        <button onClick={handleCopy} className="hover:text-white transition-all bg-white/5 hover:bg-white/10 px-3 py-1.5 rounded-lg flex items-center gap-1.5 border border-white/5 shadow-sm active:scale-95">
          {isCopied ? <Check className="w-3.5 h-3.5 text-emerald-400" /> : <Copy className="w-3.5 h-3.5" />}
          <span className="text-[10px] font-bold tracking-wider">{isCopied ? 'COPIED' : 'COPY CODE'}</span>
        </button>
      </div>
      <pre className="p-4 overflow-x-auto text-cyan-300 leading-relaxed">
        <code>{displayedCode}</code>
        {displayedCode.length < code.length && (
          <motion.span animate={{ opacity: [0, 1, 0] }} transition={{ repeat: Infinity }} className="inline-block w-1.5 h-3.5 bg-fuchsia-500 align-middle ml-1 rounded-full" />
        )}
      </pre>
    </div>
  );
...
export default function NeuralChatPage() {
  ...
  useEffect(() => {
    const initializeChat = async () => {
      ...
      if (user) {
        ...
        const { data: savedSessions } = await supabase.from('chat_sessions').select('*').eq('user_id', user.id).order('is_pinned', { ascending: false }).order('updated_at', { ascending: false });

        if (savedSessions && savedSessions.length > 0) {
          setSessions(savedSessions.map(dbSession => ({ id: dbSession.id, title: dbSession.title, is_pinned: dbSession.is_pinned, updatedAt: dbSession.updated_at })));
  ...
  const handleSendMessage = async () => {
    ...
    const chatHistoryForAPI = [...messages, newUserMessage]
      .filter(m => m.type === 'text' || m.type === 'code')
    ...
    try {
      ...
      // 🟢 ارسال کامل اطلاعات به بک‌اند (بک‌اند به عنوان ادمین همه چیز را در دیتابیس ذخیره می‌کند)
                  ...
                  <div className={`max-w-[88%] md:max-w-[80%] rounded-[1.5rem] p-4 md:p-5 shadow-xl relative group/text ${msg.role === 'user' ? 'bg-gradient-to-br from-indigo-600/90 to-blue-600/90 border border-white/10 text-white rounded-tr-sm' : 'bg-white/5 backdrop-blur-xl border border-white/10 text-neutral-100 rounded-tl-sm'}`}>
                    ...
                    {msg.imageUrls && msg.imageUrls.length > 0 && (
                      <div className="flex flex-wrap gap-2 mb-3">
                        {msg.imageUrls.map((img, idx) => <img key={idx} src={img} alt="User Upload" className="max-w-[200px] w-full h-auto rounded-xl object-cover border border-white/20 shadow-md" />)}
                      </div>
                    )}
                    ...
                    {msg.type === 'code' && <CodeBlock code={msg.content} />}
                    ...
                    {msg.type === 'image' && (
                      <div className="mt-2 relative rounded-xl overflow-hidden border border-white/10 bg-[#03000A] shadow-2xl group/img">
                        ...
                        <div className="absolute top-3 right-3 opacity-0 group-hover/img:opacity-100 transition-opacity duration-300">
                          <button onClick={() => handleDownloadImage(msg.content)} className="bg-black/70 hover:bg-black/90 backdrop-blur-md p-3 rounded-xl text-white flex items-center justify-center transition-all shadow-xl border border-white/10 hover:scale-105 active:scale-95">
